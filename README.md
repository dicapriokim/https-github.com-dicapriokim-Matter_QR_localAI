...
- **Vision & Reasoning 듀얼 파이프라인**: 지능형 기기 작명 및 스캔 정밀 보정을 위해 `moondream`과 `qwen-3b` 모델을 연동하여 상황 인식 연산을 실행합니다.
...
* **필수 모델 로드**: 구축된 LocalAI 서버에 아래 두 가지 모델이 정상적으로 로드되어 작동하는지 확인하십시오:
  - **텍스트 모델**: `qwen-3b` (또는 추론 연산용 동급 초경량 LLM)
...
> Matter Device Management & QR Code Backup/Restore Tool (v5.1.19)
...
- **Vision & Reasoning Models**: Enhanced context analysis with dual `moondream` and `qwen-3b` model pipeline.
...
* **Model Load**: Ensure you have successfully loaded the required models on your local AI server:
  - **Text Model**: `qwen-3b` (or equivalent lightweight text model)
...
Designed by **돼지지렁이 (PigWorm)** v.5.1.19
