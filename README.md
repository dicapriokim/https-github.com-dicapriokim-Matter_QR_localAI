...
**가이드 작성일:** 2026-05-06
**테스트 환경:** Proxmox VE (Intel N95, 가용 RAM 4GB 기준 최적화)
...
2. **General:** Hostname을 `AI-Mail-Server`로 입력 (권장)
3. **Template:** `Ubuntu 22.04` 또는 `24.04 LTS` 선택
4. **Disk:** `30GB` 할당 (OS 및 모델 파일 저장용으로 충분함)
5. **CPU:** Cores를 `4`로 지정 (N95의 4코어를 모두 활용하여 추론 성능 확보)
6. **Memory (매우 중요):** Memory `4096 MB` (4GB) 할당 (3GB 이상 할당 시 호스트 스왑 발생 위험 최소화)
7. **Network:** IPv4를 `Static`으로 지정하고 고정 IP 할당 (예: `192.168.x.50/24`) 및 게이트웨이 입력

#### 2단계: 필수 패키지 설치 (오류 사전 방지)
Ubuntu 최소 설치(Minimal) 템플릿의 경우 기본 네트워킹 및 압축 패키지가 누락되어 있으므로, LXC 콘솔에 접속하여 아래 명령을 먼저 실행합니다.
```bash
apt-get update && apt-get install -y curl zstd
```

#### 3단계: Ollama 구동 엔진 설치
엔진 다운로드용 curl을 사용해 Ollama 설치 스크립트를 기동합니다.
```bash
curl -fsSL https://ollama.com/install.sh | sh
```

#### 4단계: 외부 API 접속 허용 설정 (OLLAMA_HOST)
Ollama 서비스는 기본적으로 외부 접속을 차단(루프백 `127.0.0.1` 허용)하므로, 다른 컨테이너나 호스트에서 접근할 수 있도록 바인딩 주소를 개방합니다.
```bash
# 1. Ollama 서비스 설정 파일 편집기 실행
systemctl edit ollama.service
```
열린 설정 파일의 주석(#)이 없는 서비스 공간에 아래 코드를 추가하고 저장합니다. (저장: `Ctrl+O` -> `Enter` -> `Ctrl+X`)
```ini
[Service]
...
#### 5단계: 초경량 AI 모델 다운로드 및 구동
추론 능력이 강력하고 가벼운 모델들을 엔진에 적재시킵니다.
```bash
# 1단계 Vision 역할을 수행할 초경량 이미지 판독 모델 (약 1.5GB)
ollama pull moondream

# 2단계 Reasoning 역할을 수행할 경량 인공지능 모델 (약 2.0GB)
ollama pull qwen2.5:3b
```
다운로드가 끝나고 프롬프트(`>>>`)가 나타나면 질답 테스트를 진행한 뒤 `/bye`로 빠져나옵니다.
...
### 📧 Local AI Mail Automation Server Build Guide (Proxmox + Ollama + Qwen2.5:3b)

**Date:** 2026-05-06
**Environment:** Proxmox VE (Intel N95, Optimized for 4GB Available RAM)
**Goal:** Build a local API server running an ultra-lightweight AI (Qwen2.5:3b) to automatically classify emails without external cloud dependencies.

#### Step 1: Create Proxmox LXC Container
Allocate minimal resources to avoid system overhead while respecting hardware limits.
1. Click **[Create CT]** in the top right of the Proxmox Web GUI.
2. **General:** Set Hostname to `AI-Mail-Server` (recommended).
3. **Template:** Select `Ubuntu 22.04` or `24.04 LTS`.
4. **Disk:** Allocate `30GB` (sufficient for OS and model files).
5. **CPU:** Set Cores to `4` (utilizes all N95 cores to boost inference speeds).
6. **Memory (Critical):** Allocate Memory `4096 MB` (4GB) to prevent host swap generation.
7. **Network:** Configure IPv4 as `Static` with a fixed IP (e.g., `192.168.x.50/24`) and input Gateway.

#### Step 2: Install Required Packages
Minimal Ubuntu templates lack basic networking and compression tools. Connect via the LXC console and execute the following command:
```bash
apt-get update && apt-get install -y curl zstd
```

#### Step 3: Install Ollama Engine
Run the Ollama installer script via curl.
```bash
curl -fsSL https://ollama.com/install.sh | sh
```

#### Step 4: Allow External API Access (OLLAMA_HOST)
Ollama blocks external queries by default (binding to loopback `127.0.0.1`). Open access to other hosts:
```bash
# 1. Edit the Ollama service configurations
systemctl edit ollama.service
```
Insert the environment variables inside the clean service section:
```ini
[Service]
Environment="OLLAMA_HOST=0.0.0.0"
```
...
#### Step 5: Pull and Run Lightweight AI Models
Pull reasoning-efficient and image-friendly models.
```bash
# Vision Model for image/text reading (approx. 1.5GB)
ollama pull moondream

# Reasoning Model for classification (approx. 2.0GB)
ollama pull qwen2.5:3b
```
Verify the installation by talking to the model when the prompt (`>>>`) appears, then exit via `/bye`.
